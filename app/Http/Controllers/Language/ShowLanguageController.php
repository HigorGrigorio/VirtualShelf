...
class ShowLanguageController extends Controller implements IController
{
    use HasRecordArguments, AlertsUser;

    public function __construct(
        private readonly LoadLanguageById $loadLanguageById
    )
    {
    }
    ...
    {
        try {
            $findResult = $this->loadLanguageById
                ->setArgs([
                    'id' => $request->route('id'),
                ])
                ->execute();
            ...
            $return = back()->with(array_merge(
                    $this->getAlerts(),
                    [
                        'danger' => "Don't is possible show language"
                    ])
            );
        }
        return $return;
