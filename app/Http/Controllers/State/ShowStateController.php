...
class ShowStateController extends Controller implements IController
{
    ...
    public function handle(Request $request): Factory|Application|View|\Illuminate\Contracts\Foundation\Application|RedirectResponse
    {
        try {
            ...
            $return = view('state.show', array_merge(
                $this->getAlerts(),
                $this->getRecordArgs(),
                [
            ...
            $return = back()->with(array_merge(
                    $this->getAlerts(),
                    [
                        'danger' => "Don't is possible show state"
                    ])
            );
        }
        return $return;
